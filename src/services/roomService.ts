...
// Get published rooms
export const getPublishedRooms = async (): Promise<Room[]> => {
  try {
    const roomsCollection = collection(db, COLLECTION_NAME);
    const roomsQuery = query(
      roomsCollection,
...
// Real-time listener for published rooms
export const subscribeToPublishedRooms = (
  callback: (rooms: Room[]) => void,
): (() => void) => {
  const roomsCollection = collection(db, COLLECTION_NAME);
  const roomsQuery = query(
...
export const subscribeToAvailableRooms = (
  callback: (rooms: Room[]) => void,
): (() => void) => {
  const roomsCollection = collection(db, COLLECTION_NAME);
  const availableQuery = query(
...
// Update room show
export const updateRoomShow = async (
  id: string,
  show: boolean,
): Promise<void> => {
  console.log("show", show);
  try {
    await updateRoom(id, { show });
  } catch (error) {
    console.error("Error updating room show:", error);
    throw new Error("Failed to update room show");
  }
};
